...
Write-Host "Starting IKS Policy and Profile Creation using PowerShell" -ForegroundColor Yellow
...
while ($true) {
    $create = Read-Host -Prompt "Do you want to create an Addon Policy y/n"
    if ($create -eq "y") {
        & "./05_addOnPolicy.ps1"
    }
    if ($create -eq "n") {
